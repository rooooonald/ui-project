const renderChart1 = (colorMode) => {
  const chart1BtnGrp = document.querySelectorAll(".chart1-btnGrp");

  for (let button of chart1BtnGrp) {
    button.classList.remove("alert-btn");
    button.value === colorMode && button.classList.add("alert-btn");
  }
  ...
  var margin_chart1 = { top: 60, right: 30, bottom: 20, left: 30 };
  ...
  d3.csv("/dashboard/data/chart1.csv").then((data_chart1) => {
    ...
    legend_chart1
      ...
      .on("mouseover", function (event_legend, d_legend) {
        // Dim all slices except the one corresponding to the legend item
        svg_chart1
      ...
      .on("mouseout", function () {
        // Reset the opacity and stroke-width of all slices
